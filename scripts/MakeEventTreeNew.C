/*********************************************************
 * macro: MakeEventTreeNew.C                             *
 * Sort and build event correlations from raw TTree      *
 *   Version 1.3    10/4/2026   J.Smallcombe             *
 *                                                       *
 * Usage:                                                *
 * root -l 'MakeEventTreeNew.C++O("/Path/Run1_out.root")'*
 * or                                                    *
 * .L MakeEventTreeNew.C++O                              *
 * MakeEventTreeNew("/Path/Run1_out.root");              *
 * -> Read Run1_out.root  etc  
 * -> 			_1.root etc loaded automatically         *
 * -> Write /Path/Run1_out_events.root                   *
 * -> Write /Path/Run1_out_events_1.root  if >2GB        *
 *                                                       *
 * MakeEventTreeNew(InName,OutName,              		 *
 *                  ChainModeBool,BuildWindow_ns,BufferSize)*
 *                                                       *
 *  If input is split over multiple files,               * 
 * 		using chainmode should recover some events       *
 ********************************************************/
...
// =========================
const size_t BUFFERDEFAULTSIZE=2'000'000;


void ProcessChainBufferedDefault(TChain* chain, TTree* outtree,Long64_t tdiff=2000,size_t BUFFER=BUFFERDEFAULTSIZE)
{
...
void ProcessChainBuffered(TChain* chain, TTree* outtree,Long64_t tdiff=2000,int BUFFER=-1){
    if(BUFFER<0)BUFFER=BUFFERDEFAULTSIZE;
                     ...
                     int BufferSize=-1)
{
    ...
    std::vector<TString> files = GetFileList(infilename);
    ...
    if (chainmode) {
        ...
        if(chunkSize<0){
            ...
            if(std::abs(chunkSize)*100>BUFFERDEFAULTSIZE){
                std::cout << "Default Buffer Size will be too small! = " << BUFFERDEFAULTSIZE << std::endl;
